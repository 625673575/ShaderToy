...
void getRay(in float time, in vec2 fragCoord, in vec2 resolution, in vec4 mouse, inout vec3 ro, inout vec3 rd)
{
    ...
    rd = mul(cam, normalize(vec3(p, CAMERA_FL)));
}
...
vec4 saveCamera(in float time, in vec2 fragCoord, in vec4 mouse)
{
    ...
    if (abs(fragCoord.x - 4.5) < 0.5)
        fragColor = vec4(cam[2].r, cam[2].g, cam[2].b, - dot(cam[2], ro));
    if (abs(fragCoord.x - 3.5) < 0.5)
        fragColor = vec4(cam[1].r, cam[1].g, cam[1].b, -dot(cam[1], ro));
    if (abs(fragCoord.x - 2.5) < 0.5)
        fragColor = vec4(cam[0].r, cam[0].g, cam[0].b, -dot(cam[0], ro));
    
    return fragColor;
}

vec2 reprojectPos(in vec3 pos, in vec2 resolution, in Texture2D storage)
{
    mat4 oldCam = mat4(texelFetch(storage, ivec3(2, 0, 0), 0),
                        texelFetch(storage, ivec3(3, 0, 0), 0),
                        texelFetch(storage, ivec3(4, 0, 0), 0),
                        vec4(0.0, 0.0, 0.0, 1.0));

    vec4 wpos = vec4(pos, 1.0);
    vec3 cpos = mul(wpos , oldCam).xyz;
