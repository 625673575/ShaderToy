...
vec3 noised(in vec2 x)
{
    ...
    vec3 r;
    r.yz = 6.0 * f * (1.0 - f) * (vec2(b - a, c - a) + (a - b - c + d) * u.yx);
    r.x = a + (b - a) * u.x + (c - a) * u.y + (a - b - c + d) * u.x * u.y;
    return r;
}

static const mat2 m2 = mat2(1.6, -1.2, 1.2, 1.6);
...
vec4 render(in vec3 ro, in vec3 rd)
{
    ...
    for (int i = 0; i < 128; i++)
    {
        ...
        if (abs(h) < (0.003 * t) || t > tmax)
            break; // use abs(h) to bounce back if under terrain
        t += .9 * h;
    }
...
vec4 mainImage(in vec2 fragCoord)
{
    ...
    else
    {
        ...
        {
            vec4 ocol = texture(iChannel0, spos);
