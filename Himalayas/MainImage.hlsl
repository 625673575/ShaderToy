...
//
#define CLOUD_MARCH_STEPS 0
#define CLOUD_SELF_SHADOW_STEPS 1
...
{
    float gg = g * g;
    return (1. - gg) / pow(max(0,1. + gg - 2. * g * sundotrd), 1.5);
}
...
float volumetricShadow(in vec3 from, in float sundotrd)
{
    ...
    [unroll]for (int s = 0; s < CLOUD_SELF_SHADOW_STEPS; s++)
    {
        vec3 pos = from + rd * d;
...
vec4 renderClouds(vec3 ro, vec3 rd, inout float dist)
{
    ...
    [unroll]for (int s = 0; s < CLOUD_MARCH_STEPS; s++)
    {
        vec3 p = ro + d * rd;
...
bool mouseChanged()
{
    return iMouse.z * texelFetch(iChannel1, ivec3(1, 0, 0), 1).w < 0.;
}
...
vec4 mainImage(in vec2 fragCoord)
{
    ...
    if (fragCoord.y < 1.5)
    {
        ...
        if (abs(fragCoord.x - 0.5) < 0.5)
            fragColor = mouseChanged() ? vec4(0) : vec4(iResolution.xy, 0, 0);
    }
    else
    {
        ...
        {
            float dist = texelFetch(iChannel2, ivec3(fragCoord.x,fragCoord.y,0), 0).w * SCENE_SCALE;
            ...
            else
            {
                ...
                {
                    vec4 ocol = texture(iChannel1, spos).xyzw;
