...
static const vec4 cHashA4 = vec4(0., 1., 57., 58.);
static const vec3 cHashA3 = vec3(1., 57., 113.);
...
static int idObj;
static float tCur, stPitch, rotAng, rOut, rIn, rWid;
static vec3 qHit, ltPos[2];
static int doLt;
static const float dstFar = 20.;
...
float ObjDf(vec3 p)
{
    ...
    static const int ins = 12;
    static const int inh = 6;
    dMin = dstFar;
    ns = float(ins);
    nh = float(inh);
    sh = 0.2 * stPitch / ns;
...
vec3 ObjNf(vec3 p)
{
    static const vec3 e = vec3(0.001, -0.001, 0.);
...
vec3 ObjCol(vec3 ro, vec3 rd, vec3 vn)
{
    ...
    {
        if (abs(vn.y) > 0.005)
            col = 0.4 * vec3(0.5, 0.4, 0.3);
        else
            col = BrickCol(vec2(20. * (atan(qHit.z, qHit.x) / pi + 1.),
       40. * qHit.y));
    }
    else if (idObj == 3)
    {
...
float GlowCol(vec3 ro, vec3 rd, float dstHit)
{
    ...
    for (int j = 0; j < 2; j++)
    {
        ...
        ltDir /= ltDist;
        if (ltDist < dstHit)
            wGlow += pow(max(dot(rd, ltDir), 0.), 1024.) / ltDist;
    }
...
vec4 mainImage(in vec2 fragCoord)
{
    ...
    vuMat = mul(mat3(1., 0., 0., 0., vEl.x, -vEl.y, 0., vEl.y, vEl.x),
     mat3(vAz.x, 0., vAz.y, 0., 1., 0., -vAz.y, 0., vAz.x));
    rd = mul(normalize(vec3(uv, 1.5)) , vuMat);
    return vec4(ShowScene(ro, rd), 1.);
}
