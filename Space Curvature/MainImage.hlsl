...
vec3 fancyCube(Texture2D sam, in vec3 d, in float s, in float b)
{
...
static const vec4 sph1 = vec4(0.0, 0.0, 0.0, 1.0);
...
vec4 mainImage(in vec2 fragCoord)
{
    ...
    vec3 rd = normalize(mul(cam, vec3(p, -2.0)));
...
vec4 mainVR(in vec2 fragCoord, in vec3 fragRayOri, in vec3 fragRayDir)
{
    float zo = 1.0 + smoothstep(5.0, 15.0, abs(iTime - 48.0));
    ...
    return vec4(render(ro + mul(cam, fragRayOri),
                                   mul(cam, fragRayDir)), 1.0);
